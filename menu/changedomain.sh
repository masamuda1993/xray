...
changedomain_menu() {
  ...
  echo -e "  ${WHITE}5.${NC} Konfigurasi SSHWS akan diperbarui otomatis"
  echo -e "  ${CYAN}$LINE${NC}"
  echo ""
  ...
  # Update SSHWS config jika terinstall
  if [[ -f /etc/sshws/config.json ]]; then
    echo -e "  ${CYAN}[*]${NC} Mengupdate konfigurasi SSHWS..."
    sed -i "s/$current/$new_domain/g" /etc/sshws/config.json 2>/dev/null
  fi
  ...
  # Restart SSHWS jika terinstall
  if [[ -f /usr/local/bin/sshws ]]; then
    echo -e "  ${CYAN}[*]${NC} Merestart SSHWS..."
  ...
  [[ -f /usr/local/bin/sshws ]] && \
    echo -e "  ${GREEN}[✓]${NC} Konfigurasi SSHWS diperbarui"
  echo -e "  ${CYAN}$LINE${NC}"
  echo ""

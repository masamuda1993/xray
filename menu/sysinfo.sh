...
sysinfo_menu() {
  ...
  if [[ -f /usr/local/bin/sshws ]]; then
    systemctl is-active --quiet sshws \
      && sshws_st="${GREEN}● RUNNING${NC}" || sshws_st="${RED}● STOPPED${NC}"
  else
    sshws_st="${YELLOW}● NOT INSTALLED${NC}"
  fi
  ...
  echo -e "  ${YELLOW}SSHWS      ${NC}: $sshws_st"
  ...
  echo -e "  ${CYAN}$LINE${NC}"
  echo -e "  ${YELLOW}HTTP/nTLS  ${NC}: ${WHITE}80${NC}  (VMess nTLS, VLess nTLS, SSHWS nTLS)"
  echo -e "  ${YELLOW}HTTPS/TLS  ${NC}: ${WHITE}443${NC} (VMess TLS, VLess TLS, SSHWS TLS)"
